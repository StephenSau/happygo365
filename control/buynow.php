...
class buynowControl extends BaseHomeControl {
	...
	*/
	public function indexOp(){
		...
		//多件商品不能超过一千块并且不是汽车馆的商品
		if($legalGoods_info['store_goods_price'] > 1000 and $quantity > 1 and $store_id != 20)
		{
        ...
        if(C('voucher_allow') == 1 && $mansong['mansong_flag'] == false && $legalGoods_info['xianshi_flag'] ==false){
            //var_dump($legalGoods_info['goods_info'][0]['spec_goods_price']);die;
            //获取代金券列表
            $model = Model();
            $where = array();
          ...
          //  $where['voucher_store_id'] = "{$legalGoods_info['store_info']['store_id']}";
            ...
            $where['voucher_limit'] = array('elt',$legalGoods_info['goods_info'][0]['store_goods_price']);
	...
	public function bundlingOp(){
		...
		$legalGoods_info = $this->getLegalBundlingGoods($_POST['spec'], $quantity, $bl_id);
                $store_id = $legalGoods_info['goods_info'][0]['store_id'];
		...
		foreach($legalGoods_info['goods_info'] as $key=>$val)
		{
			...
			if($legalGoods_info['goods_info'][0]['total_ems'] >50)
			{
				$legalGoods_info['goods_info'][0]['store_goods_price'] = $legalGoods_info['store_goods_price']+$legalGoods_info['goods_info'][0]['total_ems'];
				
			}else
			{
				$legalGoods_info['goods_info'][0]['store_goods_price'] = $legalGoods_info['store_goods_price'];			
			}
	...
	public function bundling_step2Op() {
		...
		$array = array();
         	...
         	foreach($legalGoods_info['goods_info'] as $key=>$val)
		{
			$gc_id = $mode_goods->where("goods_id= ".$val['goods_id']."")->find();
			$name = $cat->getGoodsClassNav($gc_id['gc_id']);
			...
			if($legalGoods_info['goods_info'][0]['total_ems'] >50)
			{
				$total_ems = $legalGoods_info['goods_info'][0]['total_ems'];
                    ...
                    if(!empty($xianshi_info)) {
                        if(intval($xianshi_info['state']) === self::XIANSHI_STATE_PUBLISHED) {
                            ...
                            if(intval($xianshi_info['end_time']) > $current_time && intval($xianshi_info['start_time']) < $current_time) {
                                $legalGoods_info['goods_info'][0]['spec_goods_price'] = ncPriceFormat($xianshi_goods['xianshi_price']);
	...
	public function step2Op() {
		...
		$quantity = intval($_POST['buynow_quantity']);//购买件数
...
//		 $mode_address->upAddress($param);
    ...
    //   print_r($legalGoods_info);exit();
        ...
        //插入车牌
        if($legalGoods_info['store_info']['store_id']=='20'){
        ...
        $model = Model();
        $daddress = $model->table('daddress')->where(array('store_id'=>$legalGoods_info['store_info']['store_id'],'is_default'=>1))->find();     
        ...
        $order_goods_array['ems']	=$array['normal_ems'];
        //商品库存扣减基数
        $order_goods_array['store_base']	= $legalGoods_info['goods_info'][0]['store_base'];
		//商品国家
        ...
        //商品明细
		 $order_goods_array['d_specvalue']	= $legalGoods_info['goods_info'][0]['spec_d_specvalue'];        

...
class member_evaluateControl extends BaseMemberControl{
    ...
    public function addOp(){
    	//订单评价输出的内容
		$order_goods_all=Model('order');
		$param['order'] = '';
		$order_all=$order_goods_all->OrderGoodsList_two($param);
		...
		if (!$_POST)
		{
			...
			//订单评价输出内容
			Tpl::output('order_all',$order_all);
		...
		}else 
		{
			...
			if ($is_goodslegal == true){
				...
				foreach ($_POST['goods'] as $k=>$v){
					if (intval($v['points'])>0 && !empty($order_goods[$k])){
						$insert_arr = array();
			...
			if ($state){
				...
				$orderlog_arr['order_state']	= Language::get('member_order_finished').','.Language::get('member_order_evaluated');

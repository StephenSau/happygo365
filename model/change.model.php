...
class changeModel{
	...
	public function getReturnGoodsList($condition){
		$condition_str = $this->getCondition($condition);
		$param = array();
		$param['field'] = $condition['field'] ? $condition['field'] : 'return_goods.*,change.add_time,change.order_sn,change.return_sn';
		$param['table'] = 'change,change_goods';
		$param['where']	= $condition_str;
		$param['join_type']	= 'left join';
		$param['join_on']	= array('change.return_id=change.return_id');
		$param['order'] = $condition['order'] ? $condition['order'] : 'change.return_id desc';
		$result = Db::select($param);
		return $result;
	}
	...
	public function addGoods($param){
		...
		if (is_array($param)){
			$result = Db::insert('change_goods',$param);
			return $result;
		}else {
			return false;
		}
	...
	public function update($condition,$param){
		...
		$where = $condition_str;
		if (is_array($param)){
			$result = Db::update('change',$param,$where);
			return result;
		}else {
			return false;
		}
	...
	public function getCount($condition) {
		$condition_str	= $this->getCondition($condition);
		$count	= Db::getCount('change',$condition_str);
		return $count;
	}
	...
	private function getCondition($condition_array){
		...
		if($condition_array['return_id'] !== null) {
			$condition_sql	.= " and change.return_id = '".$condition_array['return_id']."'";
		}
		if($condition_array['order_id'] !== null) {
			$condition_sql	.= " and change.order_id = '".$condition_array['order_id']."'";
		}
		...
		if ($condition_array['keyword'] !== null){
			$condition_sql .= " and change.". $condition_array['type'] ." like '%". $condition_array['keyword'] ."%'";
		}
		if($condition_array['order_ids'] !== null) {
			$condition_sql	.= " and change.order_id in(".$condition_array['order_ids'].")";
		}

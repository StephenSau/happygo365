		...
		<div class="buyerorderlistin">
			 ...
			 <?php foreach ($output['order_array'] as $val) { ?>
				<li>
					<div class="buyerordermess">
						...
						<a target="_blank" href="index.php?act=member&op=show_order&order_id=<?php echo $val[0]['order_id']; ?>">查看订单</a>
							...
							<tr>
								<td style="width:39%" class="td-buyerordername">
									<a class="a-buyerordername" href="index.php?act=goods&goods_id=<?php echo $v['goods_id'];?>" target="_blank">
										<img src="<?php echo thumb($v,'mid'); ?>" onload="javascript:DrawImage(this,60,60);"/>
          ...
          <?php } ?>
							...
							<td rowspan="<?php echo count($val);?>" style="width:20%" class="td-bl">
								<p class="goods-price" id="order<?php echo $val[0]['order_id']; ?>_order_amount" style="padding-bottom:5px;" ><strong><?php echo $val[0]['order_amount']; ?></strong></p>
								  <?php if ($val[0]['payment_name']) { ?>
								  <p class="goods-pay" title="<?php echo $lang['member_order_pay_method'].$lang['nc_colon'];?><?php echo $val[0]['payment_name']; ?>" style="padding-bottom:5px;" ><?php echo $val[0]['payment_name']; ?></p>
								  <?php } ?>
							...
							<!--等待付款-->	
							<td  class="td-bl" style="width:15%"  rowspan="<?php echo count($val);?>">
								<p>
								<?php echo $val[0]['state_info']; ?><br/>
							  ...
							  <p style="padding-top:5px;"><a class="ncu-btn2 mt5" href="index.php?act=member_evaluate&op=add&order_id=<?php echo $val[0]['order_id']; ?>"><?php echo $lang['member_order_want_evaluate'];?></a><!--我要评价--></p>
							  <?php } ?>
							</td>
								<?php }?>
							</tr>

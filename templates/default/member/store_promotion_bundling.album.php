<div class="goods-gallery" nctype="bundling_album" style="width:625px;"> <a class="sample_demo" id="select_s" href="index.php?act=store_album&op=pic_list&item=bundling_add" style="display:none;"><?php echo $lang['nc_submit'];?></a>
  <div style="width:593px;height:40px;line-height:40px;"><span class="l"><?php echo $lang['store_goods_album_users'];?> >
  ...
  <ul class="list" style="width:610px;">
    ...
    <li onclick="bundling_insert('<?php echo cthumb($v['apic_cover'],'small',$_SESSION['store_id']);?>', '<?php echo $v['apic_cover'].'_small.'.get_image_type($v['apic_cover'])?>');" style="padding: 4px 0 4px 3px;"><a href="JavaScript:void(0);"><span class="thumb size90"><i></i><img src="<?php echo cthumb($v['apic_cover'],'tiny',$_SESSION['store_id']);?>" onload="javascript:DrawImage(this,90,90);" title='<?php echo $v['apic_name']?>'/></span></a></li>
    <?php }?>
    <?php }else{?>
...
$(document).ready(function(){
	$('div[nctype="bundling_album"]').find('.demo').unbind().ajaxContent({
		event:'click', 
		loaderType:"img",

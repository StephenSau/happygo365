  ...
  <form method="get" action="index.php" id="theForm" target="_self">
    <table class="search-form">
      ...
      <tr>
        ...
        <td class="w60"><input type="text" class="text" name="order_sn" value="<?php echo trim($_GET['order_sn']); ?>" /></td>
        <th style="width: 60px;"><?php echo $lang['store_order_buyer'].$lang['nc_colon'];?></span></th>
        <td class="w60"><input type="text" class="text" name="buyer_name" value="<?php echo trim($_GET['buyer_name']); ?>" style="margin-right: 15px;"/></td>
        <th>
          <select id="select_way">
            <option value="add_time" <?=$_GET['add_time_from']||$_GET['add_time_to'] ? 'selected':''; ?> ><?php echo $lang['store_order_add_time'];?></option>
            <option value="pay_time" <?=$_GET['pay_time_from']||$_GET['pay_time_to'] ? 'selected':''; ?> >付款时间</option>
          </select>
        </th>
        <td><input type="text" class="text" name="add_time_from" id="add_time_from" value="<?=empty($_GET['add_time_from'])?$_GET['pay_time_from']:$_GET['add_time_from']; ?>" />
          &#8211;
          <input id="add_time_to" class="text" type="text" name="add_time_to" value="<?=empty($_GET['add_time_to'])?$_GET['pay_time_to']:$_GET['add_time_to']; ?>" /></td>
        ...
        <?php if($_GET['state_type']=='order_no_shipping' || $_GET['state_type']=='order_shipping'){?><br>
        <a class="btns" href="javascript:void(0)" onclick="$('#excelch').val(1);$('#theForm').submit();">导出EXCEL</a>
        <?php }?>
        </td>
      ...
      <tr>
        <th colspan="20"><span class="fl ml10"><?php echo $lang['store_order_order_sn'].$lang['nc_colon'];?><span class="goods-num"><em><?php echo $val[0]['order_sn']; ?></em>
          ...
          <?php }?>
				  ...
				  <?php }?>
				  <!--添加提示-->
		  <a href="index.php?act=store&op=show_order&order_id=<?php echo $val[0]['order_id']; ?>" target="_blank" class="nc-show-order fl"><i></i><?php echo $lang['store_order_view_order'];?></a>
      ...
      <tr>
        ...
        <td class="bdl bdr" rowspan="<?php echo count($val);?>"><p><?php echo $v['state_info']; ?>
          ...
          <?php if ($v['payment_direct'] == '1' or extension_loaded('openssl')){?>
			...
			<?php if($v['deliver'] == 2 && $v['order_state'] ==20){?>
				<p><a class="btn bg-orange" href="index.php?act=deliver&op=send&order_id=<?php echo $v['order_id']; ?>"/><?php echo $lang['store_order_send'];?></a></p>
			<?php }?>
...
$(function(){
  ...
  $("#select_way").bind("change",function(){
    var oSelectVal = $(this).val();
    var add_time_from = $("#add_time_from")
        add_time_to = $("#add_time_to");

    if(oSelectVal == "add_time"){
      add_time_from.attr("name","add_time_from");
      add_time_to.attr("name","add_time_to")
    }else{
      add_time_from.attr("name","pay_time_from");
      add_time_to.attr("name","pay_time_to")
    }

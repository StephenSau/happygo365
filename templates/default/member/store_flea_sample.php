...
<div class="goods-gallery"> <a class='sample_demo' id="select_submit" href="index.php?act=flea_album&op=pic_list&item=goods" style="display:none;"><?php echo $lang['nc_submit'];?></a>
  ...
  <ul class="list">
    ...
    <li onclick="insert_img(<?php echo $v['apic_id'];?>,'<?php echo ATTACH_GOODS.DS.$_SESSION['store_id'].DS.$v['apic_cover'].'_small.'.get_image_type($v['apic_cover']);?>');"><a href="JavaScript:void(0);"><span class="thumb size90"><i></i><img src="<?php echo SiteUrl.DS.ATTACH_GOODS.DS.$_SESSION['store_id'].DS.$v['apic_cover'].'_small.'.get_image_type($v['apic_cover'])?>" onload="javascript:DrawImage(this,90,90);" onerror="this.src='<?php echo ATTACH_COMMON.DS.$GLOBALS['setting_config']['default_goods_image'];?>'" title='<?php echo $v['apic_name']?>'/></span></a></li>
    <?php }?>
    <?php }else{?>

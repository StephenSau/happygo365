...
<div class="wrap">
  ...
  <form name="deliver_form" method="POST" id="deliver_form" action="index.php?act=deliver&op=send" onsubmit="ajaxpost('deliver_form', '', '', 'onerror');return false;">
    ...
    <input type="hidden" value="<?php echo $output['order_array']['consignee_id_num'];?>" name="scard" id="scard">
    <table class="ncu-table-style order deliver">
      ...
      <tbody>
        ...
        <tr>
          ...
          <!--<strong class="fl"><?php echo $lang['store_deliver_buyer_adress'].$lang['nc_colon'];?></strong>
		  <?php echo $output['order_array']['area_info'];?>&nbsp;<?php echo $output['order_array']['address'];?>&nbsp;<?php echo $output['order_array']['zip_code'];?>&nbsp;<?php echo $output['order_array']['true_name'];?>&nbsp;<?php echo $output['order_array']['tel_phone'];?>&nbsp;<?php echo $output['order_array']['mob_phone'];?><a href="javascript:void(0)" nc_type="dialog" dialog_title="<?php echo $lang['store_deliver_modfiy_address'];?>" dialog_id="my_address_edit" uri="index.php?act=deliver&op=buyer_address&order_id=<?php echo encrypt($output['order_array']['order_id'],md5(1));?>" dialog_width="550" class="fr"><?php echo $lang['store_deliver_modfiy_address'];?></a>-->
          
        <div id="region">
		<?php echo $lang['store_deliver_buyer_adress'].$lang['nc_colon'];?>
        <input type="hidden" value="<?php echo $output['order_array']['city_id'];?>" name="city_id" id="city_id">

        <input type="hidden" name="area_id" id="area_id" value="<?php echo $output['order_array']['area_id'];?>" class="area_ids" />

        <input type="hidden" name="area_info" id="area_info" value="<?php echo $output['order_array']['area_info'];?>" class="area_names" />

        <?php if(!empty($output['order_array']['area_id'])){?>

        <span class="total_address"><?php echo $output['order_array']['area_info'];?></span>
        ...
        <?php }?>
        &nbsp;<input style="height:25px;width:250px;line-height:25px;text-indent:10px" name="addressedit" value="<?php echo $output['order_array']['address'];?>" placeholder="地址" />&nbsp;<input style="height:18px;" name="zip_codeedit" value="<?php echo $output['order_array']['zip_code'];?>" placeholder="邮政编码" type="hidden" />&nbsp;<?php echo $output['order_array']['true_name'];?>&nbsp;<?php echo $output['order_array']['tel_phone'];?>&nbsp;<?php echo $output['order_array']['mob_phone'];?><input type="button" id="save_address" value="保存" />
          </div>
    ...
    <table class="ncu-table-style order" id="texpress1">
      <tbody>
        ...
        <tr>
          <td class="bdl"><?php echo $output['express_list'][$v-1]['e_name'];?></td>
...
$(function(){
	...
	$("#save_address").bind("click",function(){
		var sarea_info = $("[name = 'sarea_info']");
		var saddress = $("[name = 'saddress']");
		var szip_code = $("[name = 'szip_code']");
		
		var total_address =$.trim($("#area_info").val());
		var addressedit =$.trim($("[name = 'addressedit']").val());
		var zip_codeedit =$.trim($("[name = 'zip_codeedit']").val());
		

		sarea_info.val(total_address);
		saddress.val(addressedit);
		szip_code.val(zip_codeedit);
		
		/*发送ajax东西*/
		$.post("index.php?act=deliver&op=saveaddressajax",$("#deliver_form").serialize(),function(data){
			if(data=='s'){
				alert("保存成功");
			}else{
				alert("保存失败");

...
<div class="side-message">
  ...
  <div class="guest-form">
    <form id="send_form" action='index.php?act=home&op=savemsg&type=sns_board' method="post">
      <input type="hidden" value="ok" name="form_submit">
      <input type="hidden" name="msg_type" value="2" />
      <input type="hidden" value="<?php echo $output['member_info']['member_name'];?>" name="to_member_name">
      <textarea id="content_msg" name="msg_content" placeholder="<?php echo $lang['sns_message_content_placeholder'];?>@<?php echo !empty($output['member_info']['member_truename'])?$output['member_info']['member_truename']:$output['member_info']['member_name'];?><?php echo $lang['sns_talk'];?>~" class="msg-content"></textarea>
  ...
  <div nctype="message_list" class="message-list">
    ...
    <dl nctype="dl<?php echo $val['message_id'];?>">
      <dt><a href="index.php?act=member_snshome&mid=<?php echo $val['from_member_id'];?>"><?php echo $val['from_member_name'].$lang['nc_colon'];?></a><span><?php echo parsesmiles($val['message_body']);?></span></dt>
      <dd data-param="{'msgid':'<?php echo $val['message_id'];?>','fname':'<?php echo $val['from_member_name'];?>'}"><span class="time"><?php echo $val['message_time'];?></span><span class="handle">
        ...
        <a  onclick="ajax_get_confirm('<?php echo $lang['nc_common_op_confirm'];?>', 'index.php?act=home&op=dropcommonmsg&drop_type=sns_msg&message_id=<?php echo $val['message_id'];?>');" href="javascript:void(0);"><?php echo $lang['nc_delete'];?></a>
        <?php }?>
        </span> </dd>
	...
	$('a[nctype="reply_msg"]').live('click', function(){
		...
		if(!p_dd.next().is('dd[nctyoe="replyform"]')){
			$('<dd nctyoe="replyform" class="re-msg"></dd>')
				.append('<i></i>')
				.append('<form id="replyform'+data.msgid+'" action="index.php?act=home&op=savereply" method="post"></form>').children('form')
...
function leaveMsgSuccess(data){
	$('<dl></dl>')
		...
		.append('<dd data-param="{\'msgid\':\''+data.msg_id+'\',\'fname\':\''+data.from_member_name+'\'}"><span class="time"><?php echo $lang['sns_just'];?></span><span class="handle"><a href="javascript:void(0);"onclick="ajax_get_confirm(\'<?php echo $lang['nc_common_op_confirm'];?>\', \'index.php?act=home&op=dropcommonmsg&drop_type=sns_msg&message_id='+data.msg_id+'\');"><?php echo $lang['nc_delete'];?></a></span></dd>')
